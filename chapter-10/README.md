...
```
docker compose exec -it postges psql -U postgres
```

Then, activate the extension:

```sql
CREATE EXTENTION IF NOT EXISTS vector;

\dx;
```

Let's create table to store our embeddings and insert some data to it.
...
docker compose exec -T postgres psql -U postgres < ./reviews.sql
...
Access the notebook, run the `create-embeddings` notebook.

Then, create an index to speed up vector searches.

```sql
CREATE INDEX ON reviews USING ivfflat (review_embedding vector_cosine_ops);
```
You can use `\di;` to verify.
